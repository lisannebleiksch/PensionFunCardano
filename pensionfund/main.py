...
def main() -> None:
    ...
    global liabilities_pricer  # global so it can be accessed in calculate_hedge_ratio_over_time
    ...
    all_hedging_results = perform_hedging_analysis(
        df_bond_metrics, dv01_liabilities, df_hedge, df_bonds, flat_interest_rate
    )
...
def load_and_process_data() -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, float]:
    ...
    file_path = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "data", "data_cardano.xlsx"
    )
...
def perform_hedging_analysis(
    ...
    for _, bond_row in df_bond_metrics.iterrows():
        ...
        hedge_ratio_over_time = calculate_hedge_ratio_over_time(
            bond_name, notional, df_hedge, bond_details, flat_interest_rate, hedging
        )
        ...
        hedge_ratio_over_time.append(
            {
                ...
                "bond_dv01": current_bond_dv01,  # Add this for debugging
                "liability_dv01": current_liability_dv01,  # Add this for debugging
            }
        )
